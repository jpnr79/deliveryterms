...
- The workflow also includes an optional **Playwright** E2E job that runs `tests/e2e/preview.spec.js` when the secrets `GLPI_UI_USER`, `GLPI_UI_PASS`, and `GLPI_TEMPLATE_ID` are set; the job installs Node and Playwright on the runner and executes the preview spec. The job is configured to retain video, trace and screenshots on failure and uploads these artifacts to the workflow artifacts (look for `playwright-preview-artifacts`). The Playwright job now also generates an **HTML report** (`playwright-report/index.html`) which is included in the uploaded artifact for easy offline inspection. When artifacts are uploaded, the integration workflow will attempt to find any Pull Requests associated with the run commit and insert/update a short artifacts link in the PR body (between `<!-- deliveryterms-artifacts:start -->` and `<!-- deliveryterms-artifacts:end -->`) so reviewers have a quick link to the run's artifacts page. If available, the workflow will prefer a direct download link to the extracted HTML report artifact (`playwright-html-report`) so reviewers can quickly download the report; otherwise it falls back to the run's artifacts listing. The workflow will also deploy the extracted HTML to GitHub Pages as a temporary preview (if Pages deployment is allowed in the repository) and insert the Pages URL into the PR body when available. You can enable historical previews (keep past snapshots under `/preview/<timestamp>-<runId>/index.html`) by setting the repository variable or environment variable `PAGES_PREVIEW_KEEP_HISTORY=true` in the workflow or repository settings; when enabled each run will publish its report under a timestamped subpath and the PR body will link to that snapshot. Configure retention by setting `PAGES_PREVIEW_RETENTION_DAYS` (default: 30). The workflow will attempt to prune previews older than this retention interval when it can access the Pages branch (best-effort). The CI also runs a **smoke-check** on the extracted HTML report (verifies file exists, minimum size and basic markers) before uploading it to help catch bad or truncated reports early. It additionally runs a headless Playwright validation that opens the extracted HTML report in a headless browser and checks for expected selectors (title, main content or headings) to ensure the report is viewable. The Playwright job (and Node setup) also use **npm caching** (`actions/cache`) to speed up Node/Playwright installs when a `package-lock.json` is present. A minimal `package.json` and `package-lock.json` including `@playwright/test` are included in the repo so CI caching works out of the box.
